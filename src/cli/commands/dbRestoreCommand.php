...
final class dbRestoreCommand extends Command {
	...
	protected function configure(): void {
		$this->addOption( 'from', null, InputOption::VALUE_REQUIRED, 'Source environment variant (resolves app/config/environment.json with the app/config/{from}.env overlay)', 'prod', envCommand::suggestEnvironments( ... ) );
		$this->addOption( 'to', null, InputOption::VALUE_REQUIRED, 'Target environment variant (resolved with the app/config/{to}.env overlay). Omit to use the active environment.', '', envCommand::suggestEnvironments( ... ) );
		...
		$this->setHelp( 'Cross-platform replacement for the per-app restore-live-to-local.ps1: connection strings come from app/config/environment.json, resolved per variant with the app/config/{name}.env overlay files, instead of being hardcoded. Validate an overlay first with `gf env <name>`. Requires the MongoDB Database Tools (mongodump/mongorestore) on PATH.' );
	}
	...
	protected function execute( InputInterface $input, OutputInterface $output ): int {
		...
		if( $targetConfig->type==='prod' && !$input->getOption( 'allow-prod' ) ) {
			throw new cliException( 'Refusing to restore into an environment with type "prod" (' . $context->describeEnvironmentConfigSource( $toVariant ) . '). Pass --allow-prod if you really mean it.' );
		}
